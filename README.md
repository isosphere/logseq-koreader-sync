...
Known to work with KOReader 2026.03.  
Built with a "book folder" metadata location (see https://github.com/koreader/koreader/pull/10074) in mind.[^1] 

[^1]: Settings Gear -> Document -> Book metadata location -> book folder.

This is a tool to import your annotations from KOReader in a read-only format - this is a one directional synchronization[^2]. It is not affiliated with the KOReader project.
...
[^2]: perhaps one day we can do bidirectional synchronization, but that sounds like a very difficult task right now. 
